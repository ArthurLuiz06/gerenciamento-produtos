...
async function listarProdutos() {
    ...
    try {
        ...
        produtos.forEach(produto => {
            ...
            card.className = 'produto-card'; // 💡 APENAS A CLASSE CSS

            // MONTAGEM DO HTML COM A IMAGEM E DADOS
            card.innerHTML = `
                ${produto.IMAGEM_URL ? 
                    // 💡 Adicionando uma classe para a imagem
                    `<img src="${produto.IMAGEM_URL}" alt="${produto.NOME}" class="produto-imagem">` 
                    : '<p class="aviso-imagem">[Imagem não disponível]</p>'
                }
        ...
        listaProdutosDiv.innerHTML = `<p class="erro-conexao">Erro ao conectar com o servidor para buscar produtos.</p>`;
    }
}
