...
const produtoAdmModel = {
    ...
    async cadastrar(produto) {
        const sql = 'INSERT INTO PRODUTOS (NOME, DESCRICAO, VALOR, ESTOQUE, IMAGEM_URL) VALUES (?,?,?,?,?)';
        const params = [
            produto.nome,
            produto.descricao,
            produto.valor,
    ...
    async atualizar(id, produto) {
        const sql = 'UPDATE PRODUTOS SET NOME=?, DESCRICAO=?, VALOR=?, ESTOQUE=?, IMAGEM_URL=? WHERE IDPRODUTO=?'
        const params = [
            produto.nome,
            produto.descricao,
            produto.valor,
